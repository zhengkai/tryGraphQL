...
curl -s -d '{geo(ip:"202.130.251.3",lang:pt) {ip isp{name organization} city country{iso_code name} postal_code continent location{ metro_code time_zone latitude longitude accuracy_radius }} time}' 127.0.0.1:59999/api | jq
curl -s -d '{geo(ip:"202.130.251.3") { city } status { uptime_sec uptime_text request request_error }time}' 127.0.0.1:59999/api | jq
curl -s -d '{query:"{geo(ip:\"106.187.48.203\") {city}}"}' 127.0.0.1:59999/api | jq
curl -s -d '{geo(ip:"8.8.8.8") {city}}' http://127.0.0.1/graphql/api | jq
